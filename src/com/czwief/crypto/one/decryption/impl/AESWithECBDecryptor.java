...
public class AESWithECBDecryptor implements Decryptor {
    ...
    public String decrypt(final byte[] ciphertext, final String key) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Unable to initialize cipher with key  " + secretKey.toString(), ex);
        }
        try {
            byte[] decrypted = cipher.doFinal(ciphertext);
            return new String(decrypted);
        } catch (IllegalBlockSizeException|BadPaddingException ex) {
            throw new IllegalArgumentException("Unable to decrypt cipher text " + new String(ciphertext) + " with key " + key, ex);
        }
    }
